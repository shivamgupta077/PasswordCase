...
def strToBlock(str):
    ...
    while i < len(str):
        temp = ""
        for j in range(8):
            if i + j >= len(str): #Padding if 64 bits not completed in a block.
                temp += charToBinary('`')
            else:
...
def keyGeneration(Ckey, numberOfShift, roun):
    ...
    for i in range(56):
        if i < 28:
            ...
            right += Ckey[i]
    for i in range(numberOfShift[roun]): # Shift Left
        temp1 = left[0]
        temp2 = right[0]
        templeft = left[1:]
        tempright = right[1:]
...
def encrypt(text):
    ...
    for i in range(len(blocks)):
        ...
        for round in range(16):
            roundKey = keyGeneration(Ckey, numberOfShift, round)
            L = ""
            R = ""
            for i in range(64):
                if i < 32:
...
def decrypt(myCipher):
    ...
    for i in range(len(cipherBlocks)):
        ...
        P = initialPermutation(P)  # Initial Permutation
        for round in range(16):
            roundKey = keyGeneration(Ckey, numberOfShift, 15 - round)
            L = ""
            R = ""
            for i in range(64):
                if i < 32:
        ...
        P = finalPermutation(P)
        blocks.append(P)
        myPlainText += blockToStr(P)
    toReturn = ""
    for c in myPlainText:
        if c != '`':
            toReturn += c
    return toReturn